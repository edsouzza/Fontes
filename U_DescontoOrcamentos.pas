...
type
  T_frmDescontoOrcamentos = class(TForm)
...
var
  _frmDescontoOrcamentos: T_frmDescontoOrcamentos;
...
procedure T_frmDescontoOrcamentos.FormCreate(Sender: TObject);
...
procedure T_frmDescontoOrcamentos.FormClose(Sender: TObject; var Action: TCloseAction);
begin

    ZerarVariaveis;
    _frmOrcamento.AbrirFecharTabelas;
    Close;
    Release;
...
procedure T_frmDescontoOrcamentos.FormKeyPress(Sender: TObject; var Key: Char);
begin
...
procedure T_frmDescontoOrcamentos.FormShow(Sender: TObject);
begin

     idDoOrcamento                 := _frmOrcamento.NumOrcamento;
     valorOriginal                 :=  StrToFloat(_frmOrcamento.txtValorItem.Text);
     quantidade                    :=  StrToFloat(_frmOrcamento.edtquantidadeDoItem.text);

     edt_valorUnitarioAtual.Text   :=  _frmOrcamento.txtValorItem.Text;

end;


procedure T_frmDescontoOrcamentos.pnl_rodapeClick(Sender: TObject);
begin
...
procedure T_frmDescontoOrcamentos.CalcularValorComDesconto;
begin
...
procedure T_frmDescontoOrcamentos.edt_valorDescontoKeyDown(Sender: TObject;
         ...
         begin
              ...
              _Sql:= 'UPDATE t_orcamento SET valor_item = :pNovoUnitario, total = :pNovoTotalItem WHERE '+
               ...
               With DM_Pesq.Qry_Geral do begin
                  ...
                  ParamByName('pIDItem').Value        := StrToInt(_frmOrcamento.txtIdDoITEM.text);
               ...
               With DM_Pesq.Qry_Geral do begin
                  ...
                  SQL.Add(_Sql);
                  ParamByName('pNovoUnitario').Value  := ValorComDesconto;
                  ParamByName('pIDItem').Value        := StrToInt(edtIDItem.Text);
                  ParamByName('pNovoTotalItem').Value := novoTotalDoItem;
                  ExecSQL();
...
procedure T_frmDescontoOrcamentos.CalcularValorDescontoPorPercentual;
begin
...
procedure T_frmDescontoOrcamentos.edt_percentualDescontoKeyDown(
...
procedure T_frmDescontoOrcamentos.ZerarVariaveis;
begin
...
procedure T_frmDescontoOrcamentos.edt_valorDescontoEnter(Sender: TObject);
begin
...
procedure T_frmDescontoOrcamentos.rd_valordiretoClick(Sender: TObject);
begin
...
procedure T_frmDescontoOrcamentos.rd_porpercentualClick(Sender: TObject);
begin
