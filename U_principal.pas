...
begin
   ...
   CriarTimerBarraStatus;  //TimerBarraStatus será criado nesse procedimento
...
begin
    ...
    delete(sNomeBanco,1,45);  //encurtando a string e mostrando somente o necessário que é o nome do banco apenas
    ...
    begin
        ...
        begin
            self.Caption := 'ESTA LICENÇA TEM '+FloatToStr(QdeDiasParaExpirar)+' DIA(S) DE VALIDADE PARA '+NomeEmpresaLogada;
        end else if( QdeDiasParaExpirar ) > 5 then
        begin
            //self.Caption := 'ESTA LICENÇA TEM '+FloatToStr(QdeDiasParaExpirar)+' DIA(S) DE VALIDADE PARA '+NomeEmpresaLogada;
            self.Caption := 'ESTA LICENÇA ESTA LIBERADA PARA '+NomeEmpresaLogada+' ATÉ O DIA '+RetornarFimDegustacao;
        end;
        
    end else
    begin
    
        //FACA UMA VERIFICACAO E INFORME SE FOR LICENÇA PERMANENTE NO TOPO DO FORMULARIO PRINCIPAL
    ...
    //VERIFICANDO AS PERMISSÕES DE MENUS E BOTÕES
    //ShowMessage('nivel funcao :'+ IntToStr(NivelFuncao));
    verificarPermissoes(NivelFuncao);

    //VERIFICAR STATUS DE FRENTE DE CAIXA SE LIBERADO OU NÃO
...
begin
   ...
   texto := 'O  sistema   será  reiniciado  automaticamente, acesse   com   o  usuário  desejado  autenticando  sua  senha  de  acesso!';
...
begin
    ...
    //setando o arquivo sem atributos para posterior exclusao pois se tiver como somente leitura ou sistema não deixa excluir
    Arquivo := RetornarCaminhoPastaBackup+'SYSPDV.FDB';
    SetFileAttributes(PChar(Arquivo),0);
...
begin
    ...
    {CRIANDO/ATUALIZANDO UMA COPIA DO BANCO DE DADOS COM O NOME DE SYSDESENV => IMPORTANTE SE O ARQUIVO SYSDESENV ESTIVER ABERTO NÃO PODERÁ SER EFEITVADA A CÓPIA
     SEGUE UM FORMA DE TESTAR SE A CÓPIA ESTA SENDO FEITA COM SUCESSO}
...
begin
    ...
    Application.Terminate; //coloquei aqui por conta do menu logoff pra não fechar a aplicação quando solicitado o logoff
        ...
        _Sql := 'SELECT data, saldo_atual, status FROM caixa WHERE id_caixa = (select max(id_caixa) from caixa)';
          ...
          //ATUALIZA CAIXA APOS FECHAMENTO DO DIA ANTERIOR
           ...
           With DM_Pesq.qry_Cod do begin
            ...
            //FECHA TODOS OS REGISTROS QUE ESTÃO ABERTOS DO DIA ANTERIOR OU SEJA O QUE ESTIVER COM 'A' PASSA PARA 'F'
...
begin
      ...
      //entrando na alteração dos dados da empresa
...
begin
    ...
    begin
        ...
        Application.MessageBox('Cadastre primeiro os dados da empresa no menu consultas!','Informação do Sistema',MB_ICONEXCLAMATION);
        exit;
...
begin
      ...
      texto :=  '1° Para alterar a imagem da tela principal você deve escolher uma imagem(.GIF) de sua empresa. '#13 + #13+
                '2º Encontrar a pasta do SysPdv em C:\Arquivos de Programas\SysPdv\imagens e colocar sua logo dentro desta pasta. '#13 + #13+
          ...
          FreeAndNil(_frmCupomVenda);
           ...
           begin
                  ...
                  FreeAndNil(_frmCupomVenda);
                    ...
                    Application.MessageBox('Não foram encontrados serviços executados!','Informação do Sistema',MB_ICONEXCLAMATION);

           end;

     end;

     LogOperacoes(NomeDoUsuarioLogado, 'acesso a impressão de cupom de venda');
...
begin
     ...
     begin
        ...
        begin
               ...
               Application.MessageBox('Não foram encontrados créditos pendentes até o momento!',
               'Atenção tabela vazia...', MB_OK + MB_ICONEXCLAMATION);
               exit;
...
begin
     ...
     begin
        ...
        begin
          ...
          Application.MessageBox('É necessário que o segmento esteja cadastrado, cadastre-o primeiro!','Informação do Sistema',MB_ICONEXCLAMATION);
          exit;
...
begin
     ...
     begin
          ...
          begin
             Application.MessageBox('Você não tem produtos cadastrados,ativos ou disponíveis'#13'até o momento, primeiro efetue os cadastros  pertinentes'#13'aos  produtos  para  poder acessar  esta consulta!',
                    'Informação do Sistema!', MB_OK + MB_ICONWARNING);
...
begin
     ...
     begin
           ...
           if( TabelaEstaVazia('produtos') )then
          begin
             Application.MessageBox('Você não tem produtos cadastrados,ativos ou disponíveis'#13'até o momento, primeiro efetue os cadastros  pertinentes'#13'aos  produtos  para  poder acessar  a  esta consulta!',
                    'Informação do Sistema!', MB_OK + MB_ICONWARNING);
             btnProdutosClick(self);
          end else begin
...
begin
     ...
     begin
        ...
        begin
           Application.MessageBox('Você não tem produtos cadastrados,ativos ou disponíveis'#13'até o momento, primeiro efetue os cadastros  pertinentes'#13'aos  produtos  para  poder acessar  a  venda dos mesmos!',
                  'Informação do Sistema!', MB_OK + MB_ICONWARNING);
           btnProdutosClick(self);
        end else begin
...
begin
   // Liberando o módulo frente de caixa para o atual cliente
   logo.Visible := false;
...
begin
     ...
     begin
          ...
          FreeAndNil(_frmEscolhaParaReativacoes);
           ...
           Application.MessageBox('Não    há    clientes,   contatos,   produtos,   funcionários'#13'ou  fornecedores  inativados  até  o  presente  momento!',
                                  'Informação do Sistema!', MB_OK + MB_ICONWARNING);
...
begin   
   ...
   begin
       ...
       Application.MessageBox('Este  recurso  não  funciona no  Windows 7  atualize '#13'seu Sistema Operacional para que os vídeos possam'#13'ser executados via navegador da internet!',
   ...
   begin
     ...
     begin
         ...
         Application.MessageBox('Para ver o vídeo você precisa estar conectado á internet, conecte-se e tente novamente!',
                                'Sistema não esta conectado a internet!', MB_OK + MB_ICONWARNING);
         exit;
...
begin
   ...
   btnImprimirCupom.Hint:='2ª via de cupons na '+getImpressoraSetada;
...
begin
   ...
   AbriFecharTabelas;
    ...
    texto:= 'A Impressora padrão foi alterada para o tipo '+tipoimpr+'!';
